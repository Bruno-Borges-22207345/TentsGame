...
fun processaCoordenadas(coordenadasStr: String?, numLines: Int, numColumns: Int): Pair<Int,Int>? {
    ...
    val coordenadas:Array<String>? = (coordenadasStr?.split(","))?.toTypedArray()
    var primeiro = 0
    var segundo = 0

    if (coordenadasStr != null && coordenadas != null && coordenadasStr.length == 3) {
    ...
    do {
        ...
        do {
            ...
            if (coordenadas != null) {
                ...
                /*val coordLinha = coordenadas.first
                val coordColuna = coordenadas.second*/

                /*if (podeColocarTenda){
                    if (terreno[coordLinha][coordColuna] == "T"){
                        terreno[coordLinha][coordColuna] = null
                    }else if (terreno[coordLinha][coordColuna] == null){
                        terreno[coordLinha][coordColuna] = "T"
                    }
